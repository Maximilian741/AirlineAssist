...
const schedule = JSON.parse(JSON.stringify(sandbox.window.COC_DATA.schedule));
...
export const COC_SCHEDULE: ScheduleLeverage = ${JSON.stringify(schedule, null, 2)};
`;
writeFileSync(path.join(root, 'mobile/src/data/coc-full.ts'), ts);
console.log(`coc-full.ts: ${airlines.length} airlines, ${airlines.reduce((n, a) => n + (a.provisions || []).length, 0)} provisions, ${schedule.tactics.length} schedule tactics`);
