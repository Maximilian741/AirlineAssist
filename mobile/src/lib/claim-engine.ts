/**
 * Claim engine (TypeScript port of the web public/claim-engine.js — verified + audited).
 * Pure logic, no DOM. Maps a passenger's situation to the exact entitlements (amount + citation +
 * deadline) and generates a demand letter + DOT complaint text. Numbers verified mid-2026.
 *
 * NOTE: mirrors the web engine; keep in sync until a shared `core` package is extracted.
 */

export type ClaimType = 'cancelled' | 'schedule' | 'delayed' | 'bumped' | 'bag_late' | 'bag_lost' | 'downgrade' | 'extra';
export type Region = 'us' | 'intl_from_us' | 'from_eu' | 'from_uk' | 'canada';
...
export type Answers = {
  ...
  incidentDate?: string;
  schedDelta?: '<1' | '1-2' | '2-3' | '3-4' | '4-6' | '6+' | 'route';
  schedAccepted?: 'yes' | 'no';
  schedNotice?: '14+' | '7-13' | '<7';
  schedFrom?: string;
  schedTo?: string;
  schedDirection?: 'later' | 'earlier';
  flightDate?: string;
  [k: string]: unknown;
};
...
export type Strength = 'strong' | 'conditional' | 'info' | 'action';
...
const DB_CAP_200 = 1075;
const DB_CAP_400 = 2150;
...
type FlowStep = { id: string; when: (a: Answers) => boolean; q: Omit<Question, 'id'> };
...
const FLOW: FlowStep[] = [
  ...
  { id: 'region', when: (a) => !!a.type && a.type !== 'extra', q: { title: 'Where was the flight?', kind: 'choice', options: REGION_OPTS } },
  {
    id: 'schedDirection',
  ...
  { id: 'fareOneWay', when: (a) => a.type === 'bumped' && a.voluntary === 'no', q: { title: 'What did your one-way ticket cost?', help: 'Just your fare, one direction. A rough number is fine.', kind: 'money' } },
...
const ukAmount = (band: string | undefined, delay?: string) => (band === 'long' ? (delay === '3-4' ? 260 : 520) : band === 'medium' ? 350 : 220);
...
const delayAtLeast3 = (d?: string) => ['3-4', '4-6', '6-9', '9+'].includes(d as string);

function bumpResult(region: Region | undefined, band: string | undefined, fare: number) {
...
export function assess(a: Answers): ClaimResult {
    ...
    if (a.voluntary === 'yes') {
      ...
      if (r.pct === 0) {
        ...
        E.push({
          ...
          amountText: r.amt != null ? usd(r.amt) + ` (${r.pct}% of your one-way fare, capped at ${usd(r.cap)})` : `${r.pct}% of your one-way fare, up to ${usd(r.cap)}`,
  ...
  if (a.type === 'bag_late') {
    const pastThreshold =
      region === 'us' ? ['12-15', '15-30', '30+', 'missing'].includes(a.bagHours as string) : ['15-30', '30+', 'missing'].includes(a.bagHours as string);
    if (pastThreshold) {
      E.push({
        ...
        detail:
          ...
          (a.reportFiled === 'yes' ? 'You filed a report, so you qualify.' : 'IMPORTANT: this refund only applies if you filed a Mishandled Baggage Report at the airport. If you didn’t, you may have lost this one.'),
        rule: '14 CFR Part 260',
        ruleUrl: URL.bag260,
        deadline: 'They must pay ' + refundDeadline + '.',
      });
    }
    if (a.bagHours === 'missing') addLostBag(E, region);
  }
  ...
  return {
    entitlements: E,
    headline: buildHeadline(E),
    letterBody: buildLetter(E, a),
    dotText: buildDot(E, a),
    hasClaim: strongCount > 0 || E.some((e) => e.strength === 'conditional'),
    disclaimer: 'Estimates based on U.S. DOT rules and EU/UK/Canada regulations as of mid-2026. Confirm specifics for your trip — this is information, not legal advice.',
  };
}
...
function scheduleIsSignificant(a: Answers): boolean {
  if (a.schedDelta === 'route') return true;
  const bands = a.region === 'us' ? SCHED_SIG_DOM : SCHED_SIG_INTL;
  return bands.includes(a.schedDelta as string);
}
...
function addScheduleIntl(E: Entitlement[], a: Answers) {
  const band = a.distanceBand || 'long';
  const later3h = ['3-4', '4-6', '6+'].includes(a.schedDelta as string);
  ...
  const push = (title: string, detail: string, condition: string) => E.push({ strength: 'conditional', title, amountText: amt + ' per person, in cash', detail, condition, rule, ruleUrl, deadline });
  ...
  if (a.schedDirection === 'earlier') {
    ...
    if (!n || n === '14+') return;
    const moreThan1h = ['1-2', '2-3', '3-4', '4-6', '6+'].includes(a.schedDelta as string);
...
type IntlKind = 'cancellation' | 'delay' | 'denied_boarding';
function addIntlComp(E: Entitlement[], a: Answers, kind: IntlKind) {
  const region = a.region;
  ...
  const cond = 'You qualify only if the cause was within the airline’s control (mechanical, staffing, overbooking) — NOT extraordinary weather, ATC strikes, or security.';
  if (region === 'from_eu') {
    E.push({
      strength: 'conditional',
...
function addAmenities(E: Entitlement[]) {
  E.push({
    ...
    detail: 'If the delay/cancellation was within the airline’s control (crew, maintenance, fueling — not weather), most big U.S. airlines have promised these on the DOT dashboard. Ask for them at the gate and hold them to the promise.',
...
function addLostBag(E: Entitlement[], region?: Region) {
  if (region === 'us') {
    E.push({
      strength: 'strong',
...
function addEscalation(E: Entitlement[], a: Answers) {
  const chargeback = a.payment === 'credit';
  const bits: string[] = [];
  if (chargeback) {
    const by = a.incidentDate ? addDays(a.incidentDate, 60) : null;
...
function addDays(dateStr: string, n: number): string | null {
...
function buildHeadline(E: Entitlement[]): string {
...
function incidentSentence(a: Answers): string {
  switch (a.type) {
    case 'cancelled':
      return a.traveled === 'no' ? 'the airline canceled my flight and I chose not to travel.' : 'the airline canceled my original flight.';
    case 'schedule': {
      const SIZE: Record<string, string> = { '<1': 'under an hour', '1-2': '1–2 hours', '2-3': '2–3 hours', '3-4': '3–4 hours', '4-6': '4–6 hours', '6+': 'more than 6 hours' };
      const size = a.schedDelta === 'route' ? 'by adding a connection or changing my airport' : a.schedDelta && SIZE[a.schedDelta] ? 'by ' + SIZE[a.schedDelta] : 'significantly';
...
function buildLetter(E: Entitlement[], a: Answers): string {
...
function dotCategory(a: Answers): string {
  switch (a.type) {
    case 'cancelled':
    case 'delayed':
...
function buildDot(E: Entitlement[], a: Answers): string {
...
export function fill(text: string, d: Details, a: Answers): string {
  const map: Record<string, unknown> = {
    ...
    '[YOUR NAME]': d.name,
    '[PHONE / EMAIL]': d.email,
    '[DATE]': a.type === 'schedule' ? (a.flightDate as string | undefined) : a.incidentDate,
  };
  let out = text;
  for (const k in map) {
    const v = map[k];
    if (v) out = out.split(k).join(String(v));
  }
  return out;
}
...
/** "Final notice before legal action": a small-claims demand that pleads breach of the airline's OWN
 * contract (not preempted under Wolens, 513 U.S. 219) and states an amount and a deadline. */
export function smallClaimsNotice(a: Answers, d: Details = {}, opt: { deadlineDays?: number; amount?: string; cocRule?: string } = {}): string {
...
export function evidencePack(a: Answers): string[] {
  const base = [
  ...
  return base.concat(extra[a.type as string] || []);
}
