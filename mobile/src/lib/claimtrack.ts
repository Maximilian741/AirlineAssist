/**
 * Claim tracker — what happens AFTER you hit send (TS port of web public/claimtrack.js; parity-tested).
 *
 * Filing is the easy part. Airlines win by attrition — you file, they go quiet, weeks pass. What gets
 * results is never losing the thread: every filing on the record, every legal response clock tracked,
 * and the next escalation ready the day the airline misses its clock. The clocks are the airline's
 * legal obligations, not guesses:
 *   * written complaint → acknowledge 30 days, substantive answer 60 days (14 CFR 259.7(c))
 *   * refund owed → 7 business days (card) / 20 calendar days (other) (14 CFR 260.10)
 *   * bumping cash → at the airport, or mailed within 24h (14 CFR 250.8)
 *   * EU ~6 weeks / UK 8 weeks / Canada APPR 30 days (s.19(4)); chargeback FCBA 60 days
 *
 * Pure: the caller loads/saves the list (AsyncStorage) — every function takes and returns data.
 */
import type { Answers, Details } from '@/lib/claim-engine';

export const CLAIMS_KEY = 'ff-claims';

export type Stage = 'airline' | 'dot' | 'chargeback' | 'smallclaims';
export type ClaimStatus = 'open' | 'paid' | 'denied' | 'dropped';
export type Filing = { stage: Stage; date: string; note: string };
export type TrackedClaim = {
  id: string; created: string; answers: Answers; details: Details; headline: string; amount: string;
  filings: Filing[]; status: ClaimStatus; outcome: { date: string; note: string } | null;
};
export type Clock = { key: string; label: string; due: (filed: string) => string | null; rule: string };
...
export function addBusinessDays(d: string, n: number): string | null {
  const x = new Date(d + 'T12:00:00'); if (isNaN(x.getTime())) return null;
  let left = n;
...
export function fmt(d: string): string { const x = new Date(d + 'T12:00:00'); return isNaN(x.getTime()) ? '' : x.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }); }

export const STAGES: Record<Stage, { label: string; channel: string; clocks: (a: Answers, d: Details) => Clock[]; next: Stage[] }> = {
  airline: {
...
type ResultLike = { headline?: string; entitlements?: { strength: string; amountText?: string }[] };
...
/** Record that a filing happened through a channel. Idempotent per stage per day. Returns the updated claim. */
export function recordFiling(c: TrackedClaim, stage: Stage, note = ''): TrackedClaim {
  if (!STAGES[stage]) return c;
  const d = today();
  if (c.filings.some((f) => f.stage === stage && f.date === d)) return c;
  return { ...c, filings: [...c.filings, { stage, date: d, note }] };
}
export function markOutcome(c: TrackedClaim, status: ClaimStatus, note = ''): TrackedClaim {
  return { ...c, status, outcome: { date: today(), note } };
}

/** The live picture: for each filing, the clocks it started and their status. */
export function timeline(c: TrackedClaim): TimelineItem[] {
  const now = today();
...
export function nextAction(c: TrackedClaim): NextAction {
  ...
  const cbWindowOpen = firstFiling ? (daysBetween(firstFiling, today()) ?? 0) <= 60 : true;
...
export function summary(list: TrackedClaim[]) {
